...
import { AppText, CircleButton, Dot, Icon, PingDot, Pill, Touchable, useLayout } from '@/components/ui';
...
export default function TransitScreen() {
  ...
  const [tab, setTab] = useState<'courses' | 'transit'>('courses');
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 76, paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
        <View style={[styles.segment, { marginHorizontal: gutter }]}>
          {(
            [
              ['courses', 'Mes courses'],
              ['transit', 'Car / Bus'],
            ...
            <Touchable key={id} onPress={() => setTab(id)} style={[styles.segmentItem, tab === id && styles.segmentOn]} accessibilityRole="tab" accessibilityState={{ selected: tab === id }}>
              <AppText variant="labelMd" color={tab === id ? colors.onSurface : colors.onSurfaceVariant}>
                {label}
              </AppText>
            </Touchable>
          ))}
...
const styles = StyleSheet.create({
  ...
  segment: { flexDirection: 'row', backgroundColor: colors.surfaceContainer, borderRadius: 12, padding: 3, marginBottom: 16 },
